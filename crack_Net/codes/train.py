...
from data.augmentation import augCompose, RandomBlur, RandomColorJitter, RandomCropBoth, random_flip, RandomMove, \
    RandomRomate, Reshape
...
def main():
    ...
    # data_augment_op = augCompose(transforms=[[RandomColorJitter, 0.5], [RandomBlur, 0.2],[RandomMove,0.5],[RandomRomate,0.5],[RandomCropBoth,1],[random_flip,0.5],])
    data_augment_op = augCompose(transforms=[[RandomColorJitter, 0.5], [RandomBlur, 0.2],[RandomRomate,0.5],[RandomCropBoth,1],[random_flip,0.5],])
    # data_augment_op = augCompose(transforms=[[RandomColorJitter, 0.5], [RandomBlur, 0.2],[RandomCropBoth,1],[random_flip,0.5]])
    # data_augment_op = augCompose(
    #     transforms=[[RandomColorJitter, 0.5], [RandomBlur, 0.2],  [RandomCropBoth, 1] ])
    # data_augment_op = augCompose(
    ...
    try:
        ...
        for epoch in range(1, cfg.epoch):
            ...
            if epoch % 5== 0:
                final_results2 = metric.cal_prf_metrics(pred_list2, gt_list2, 0.01)
                max_f2 = np.nanmax(final_results2, axis=0)
                print("save_ods:" + str(max_f2[3]))
                # -------------------- val ------------------- #
            model.eval()
            val_loss = {
                'eval_total_loss': 0,
            ...
            with torch.no_grad():
                ...
                for idx, (img, lab) in enumerate(val_loader, start=1):
                    ...
                    pred_list.append(val_pred[0])
                    gt_list.append(val_target)
                ...
                # else:
                    ...
                    #
                    # })
                final_ois = metric.cal_ois_metrics(pred_list=pred_list, gt_list=gt_list, thresh_step=0.01)

                final_results = metric.cal_prf_metrics(pred_list, gt_list, 0.01)

                max_f = np.nanmax(final_results, axis=0)

                # q=max_f[3]
                # ods_str =  str(q)[0:5] + " ois: " + str(final_ois)
                print("save_ods:" + str(max_f[3]) + "  save_ois:" + str(final_ois))

                if cfg.save_ods < (max_f[3]):
                    cfg.save_ods = max_f[3]
                    cfg.saveois = final_ois
                    trainer.saver.save(model, save_ods=cfg.save_ods, saveois=cfg.saveois,
                                       tag='%s_epoch(%d)_acc(%0.5f/%0.5f)' % (
                                           cfg.name, epoch, cfg.save_ods, cfg.saveois))
                    # ----------------- save model ---------------- #
